	...
	*/
	...
	// in the child process fork() returns 0, so the following is executed in the child
		...
		if ( execv(cpath, cmd) == -1) {		// execv error
			...
			// NOTE: The below is exit() instead of return
			// This is run in the CHILD process
			// Return will casuse issues with text duplication and ultimately test failure
			// ref: https://www.coursera.org/learn/linux-system-programming-introduction-to-buildroot/discussions/weeks/2/threads/m9PkM0BUEe-GERLFz_tqKw 
			exit(EXIT_FAILURE);				
		}
	}
